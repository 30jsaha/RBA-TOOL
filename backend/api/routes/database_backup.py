...
def _staging_database(settings):
    # Retrieve environment variable, or fall back to appending '_staging' to the current DB name
    staging = os.getenv("RESTORE_STAGING_DB", "").strip()
    
    if not staging:
        # Sanitize current database name and append '_staging'
        base_name = re.sub(r"[^A-Za-z0-9_]", "_", settings["database"])
        staging = f"{base_name}_staging"[:64]
...
def _staging_restore(source, backup_id, settings, user_id):
    ...
    try:
        ...
        # Step 2: Restore decompressed SQL stream into staging
        restore_args = [mysql_bin, *_settings_args(settings), "--default-character-set=utf8mb4", staging]
        ...
        # Decompress `.gz` archive chunk-by-chunk directly into mysql stdin
        with gzip.open(source, "rb") as dump:
        ...
        if process.returncode != 0:
            raise RuntimeError(stderr_bytes.decode("utf-8", errors="replace")[:500] or "The backup could not be restored into staging.")

        # Step 3: Validate table restoration
        validate_args = [
...
# Refactored _run_dump table validation to sanitize table names for subprocess safety
def _validate_requested_tables(requested, settings):
    ...
    for name in requested:
        ...
        # Enforce strict identifier format
        if not re.fullmatch(r"[A-Za-z0-9_]{1,64}", clean_name):
            raise ValueError(f"Invalid table identifier format: {clean_name}")
        if clean_name not in available:
            raise ValueError(f"Unknown database table selected: {clean_name}")
...
def preview_restore(backup_id):
    ...
    try:
        try:
            ...
            status_code = 503 if "live database" in message or "staging database name" in message else 500
...
def restore_backup():
    ...
    if confirmation != "RESTORE_PRODUCTION":
        return jsonify({
            "success": False, 
            "message": "Direct restore requires explicit confirmation 'RESTORE_PRODUCTION'."
        }), 400

    row = db.session.execute(
        text(f"SELECT * FROM {HISTORY_TABLE} WHERE id=:id AND status='SUCCESS'"), 
        {"id": backup_id}
    ).mappings().first()
    
    if not row:
        return jsonify({"success": False, "message": "Selected backup point not found."}), 404

    settings = get_mysql_settings()
    source = _validate_backup_record(row, settings)

    if not _operation_lock.acquire(blocking=False):
        return _busy_json(DatabaseMaintenanceBusy("Another maintenance task is running."))
    ...
    try:
        ...
        emergency_filename = f"pre_restore_safety_snapshot_{stamp}.sql.gz"
        _run_dump(emergency_filename, "EMERGENCY_SNAPSHOT", user_id)

        # STEP 2: Determine which specific tables to truncate
        engine = get_mysql_engine()
        with engine.connect() as conn:
            conn.execute(text("SET FOREIGN_KEY_CHECKS=0;"))
            
            if row.get("backup_scope") == "SELECTED_TABLES" and row.get("table_list"):
                # Only truncate the specific tables included in this backup archive
                tables_to_truncate = [t.strip() for t in str(row["table_list"]).split(",") if t.strip()]
            else:
                # Full backup: fetch all existing tables in the database
                tables_to_truncate = conn.execute(text(
                    "SELECT TABLE_NAME FROM information_schema.tables WHERE TABLE_SCHEMA=:db AND TABLE_TYPE='BASE TABLE'"
                ), {"db": settings["database"]}).scalars().all()
            
            for t in tables_to_truncate:
                # Enforce safe MySQL table identifier matching
                if re.fullmatch(r"[A-Za-z0-9_]{1,64}", str(t)):
                    conn.execute(text(f"TRUNCATE TABLE `{t}`"))

            conn.execute(text("SET FOREIGN_KEY_CHECKS=1;"))
            conn.commit()

        # STEP 3: Stream backup directly into live production database
        mysql_bin = _resolve_mysql_binary("mysql", "MYSQL_PATH")
        restore_args = [
            mysql_bin, 
            *_settings_args(settings), 
        ...
        if process.returncode != 0:
            raise RuntimeError(
                ...
                or "Direct restore failed."
            )

        duration = round(time.monotonic() - started, 3)
        _audit(user_id, "DIRECT_RESTORE_SUCCESS", backup_id, "SUCCESS", duration)
        db.session.commit()

        return jsonify({
            "success": True, 
            "message": "Production database restored successfully for the targeted tables."
        })

    except Exception as exc:
        duration = round(time.monotonic() - started, 3)
        _audit(user_id, "DIRECT_RESTORE_FAILED", backup_id, "FAILED", duration, _safe_error(exc))
        db.session.commit()
        return jsonify({"success": False, "message": _safe_error(exc)}), 500
    finally:
        _operation_lock.release()