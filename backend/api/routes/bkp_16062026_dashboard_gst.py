...
def _get_period_bounds():
    ...
    start_month = int(params.get("start_month", now.month))
    end_year = int(params.get("end_year", now.year))
    end_month = int(params.get("end_month", now.month))
...
def _warn_if_gst_province_index_missing():
    ...
    try:
        rows = db.session.execute(index_sql).mappings()
        indexes = {}
        for row in rows:
...
def segmentation_distribution():
    ...
    try:
        ...
        query = text(f"""
            SELECT
                COALESCE(segmentation, 'Unknown') AS segment_label,
                COUNT(*) AS tin_count
...
def download_province_distribution():
    ...
    summary_sql = """
        SELECT
            COALESCE(NULLIF(TRIM(tr.province), ''), 'Not Available') AS province
    ...
    summary_rows = _stream_mappings(text(summary_sql), params)
    fraud_rows = _stream_mappings(text(fraud_sql), params)
