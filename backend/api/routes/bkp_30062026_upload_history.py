from flask import Blueprint, request, jsonify, make_response, send_from_directory, current_app, url_for
...
def list_uploads_by_type(tax_parameter):
    ...
    rows = db.session.execute(
        ...
        {"tax_type": str(tax_parameter).upper()},
    ).fetchall()
    return jsonify(
        [
            {
...
def download_recent_uploads_csv():
    ...
    token = _build_recent_uploads_download_token(filename)
    download_url = url_for(
        "upload_history.download_recent_uploads_csv_file",
        filename=filename,
        token=token,
        _external=True,
    )
