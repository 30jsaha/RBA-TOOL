...
SUMMARY_REQUIRED_INDEXES = {
    "idx_multitax_dashboard_summary_user_year": (False, ["user_id", "tax_period_year"]),
    "idx_multitax_dashboard_summary_user_tin": (False, ["user_id", "tin"]),
    "idx_multitax_dashboard_summary_user_sector": (False, ["user_id", "sector_activity"]),
    "idx_multitax_dashboard_summary_user_predicted_fraud": (False, ["user_id", "predicted_fraud"]),
    "idx_multitax_dashboard_summary_user_year_tin": (False, ["user_id", "tax_period_year", "tin"]),
}
...
def _summary_table_ddl(table_name):
    ...
    return f"CREATE TABLE IF NOT EXISTS {table_name} (id BIGINT NOT NULL AUTO_INCREMENT, user_id BIGINT NOT NULL, tin VARCHAR(64) NOT NULL, taxpayer_name VARCHAR(255) NULL, taxpayer_type VARCHAR(100) NULL, tax_period_year INT NOT NULL, sector_activity VARCHAR(255) NULL, enterprise_activity VARCHAR(255) NULL, total_income DECIMAL(24,2) NOT NULL DEFAULT 0, profit DECIMAL(24,2) NOT NULL DEFAULT 0, cit_tax DECIMAL(24,2) NOT NULL DEFAULT 0, gst_sales DECIMAL(24,2) NOT NULL DEFAULT 0, salary_wages DECIMAL(24,2) NOT NULL DEFAULT 0, gst_sales_diff_abs DECIMAL(24,2) NOT NULL DEFAULT 0, swt_salary_diff_abs DECIMAL(24,2) NOT NULL DEFAULT 0, predicted_fraud VARCHAR(50) NOT NULL DEFAULT 'Non-Risk Flagged', fraud_flag TINYINT(1) NOT NULL DEFAULT 0, multi_tax_issue VARCHAR(255) NULL, province VARCHAR(255) NULL, created_at DATETIME NOT NULL, updated_at DATETIME NOT NULL, PRIMARY KEY (id), UNIQUE KEY uq_summ_uty (user_id, tin, tax_period_year), KEY idx_summ_uy (user_id, tax_period_year), KEY idx_summ_ut (user_id, tin), KEY idx_summ_us (user_id, sector_activity), KEY idx_summ_upf (user_id, predicted_fraud), KEY idx_summ_uyt (user_id, tax_period_year, tin)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci"
...
def _summary_scope(include_tin=False):
    ...
    params["user_id"] = get_authenticated_user_id()
    filters = ["user_id = :user_id", f"({date_filter})"]
    if include_tin:
...
def _log_dashboard_query_context(endpoint_name, params):
    ...
    current_app.logger.info("Common dashboard %s request tin=%r range_type=%s start_date=%s end_date=%s start_year=%s end_year=%s user_id=%s", endpoint_name, meta["tin"], meta["range_type"], meta["start_date"], meta["end_date"], params.get("year", params.get("start_year")), params.get("year", params.get("end_year")), params.get("user_id"))
...
def _log_dashboard_zero_row_count(endpoint_name, include_tin, params):
    ...
    current_app.logger.warning("Common dashboard %s returned no rows. filtered_row_count=%s tin=%r start_year=%s end_year=%s user_id=%s", endpoint_name, int(total_rows), params.get("tin"), params.get("year", params.get("start_year")), params.get("year", params.get("end_year")), params.get("user_id"))
...
def _rebuild_summary_insert_sql(conn, target_table):
    ...
    return f"""
        INSERT INTO {target_table} (
            user_id, tin, taxpayer_name, taxpayer_type, tax_period_year,
        ...
        WITH base_summary AS (
            ...
            GROUP BY pr.user_id, TRIM(pr.tin), pr.tax_period_year
        )
        {registration_cte}
        SELECT
            base.user_id,
...
def _run_summary_rebuild():
    ...
    try:
        ...
        with engine.begin() as conn:
            expected_grouped_rows = int(conn.execute(text("SELECT COUNT(*) FROM (SELECT user_id, TRIM(tin), tax_period_year FROM multi_tax_integration_results WHERE user_id IS NOT NULL AND tax_period_year IS NOT NULL AND NULLIF(TRIM(tin), '') IS NOT NULL GROUP BY user_id, TRIM(tin), tax_period_year) grouped_rows")).scalar() or 0)
        _log_rebuild_step("Completed successfully", source_rows=source_rows, temp_rows=temp_rows, live_rows=live_rows, expected_grouped_rows=expected_grouped_rows, elapsed_ms=int((time.perf_counter() - rebuild_started_at) * 1000))
