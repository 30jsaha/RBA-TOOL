        ...
        try:
            if payload.get('output_dir') and payload['financial_difference_count'] > 0:
                ...
                if conflict_tins:
                    ...
                    os.close(fd)
                    try:
                        wrote_csv = _export_upload_conflicts_csv_from_db("SWT", conflict_tins, temp_csv_path)
...
def run_swt_preprocessing(saved_path, on_step=None, make_timestamped_copies=False, output_dir_override=None, upload_history_id=None):
    ...
    try:
                ...
                if unique_tins:
                    ...
                    if db_df is not None and not db_df.empty:
                        ...
                        if matched.any():
                            ...
                            try:
                                ...
                                for c in fin_cols:
                                    if c in diff_rows.columns and f"{c}__db" in diff_rows.columns:
                                        diff_fields += int(
                                            (diff_rows[c].fillna(0.0) != diff_rows[f"{c}__db"].fillna(0.0)).sum()
                                        )
                            ...
                            print("duplicate ids =", int(len(dup_upload_ids)))
                            print("financial diff ids =", int(len(diff_upload_ids)))
                            ...
                            if remove_ids:
                                ...
                                removed_rows['reason'] = [
                                    "Duplicate SWT record already exists in swt_fraud_justification" if rid in set(dup_upload_ids)
                                    else "Financial values differ from existing swt_fraud_justification record"
                                    for rid in removed_rows['_upload_row_id'].tolist()
                                ]

                                removed_rows = removed_rows.drop(columns=['_db_key', '_upload_row_id'], errors='ignore')
                                removed_df = _pd.concat([removed_df, removed_rows], ignore_index=True)

                                keep_cols = [c for c in cleaned_df.columns if c not in ['_db_key', '_upload_row_id']]
                                cleaned_df = cleaned_df.loc[~cleaned_df['_upload_row_id'].isin(remove_ids), keep_cols].copy()
        ...
        try:
            internal_cols = ['_row', '_upload_row_id', '_db_key']
