...
def enrich_taxpayer_names(df, registration_file="25.05.21.05 TIN Registrations.csv"):
    """
    Enrich taxpayer names from registration data with progress tracking
    """
    print("\n--- Step 4: Enriching taxpayer names ---")
    
    # First check for existing taxpayer name columns in cit

    existing_taxpayer_cols = []

    print("Looking for taxpayer name columns...")
    for col in tqdm(df.columns, desc="Scanning columns", unit="col"):
        col_lower = col.lower().replace('_', '')
        if col_lower in ['taxpayer', 'taxpayername', 'taxpayer_name', 'taxpayer', 'taxpayername']:
            existing_taxpayer_cols.append(col)
    
    # Determine if we need to perform the merge
    perform_merge = False
    
    if existing_taxpayer_cols:
        # Check for null values in the first taxpayer column
        taxpayer_col = existing_taxpayer_cols[0]
        null_count = df[taxpayer_col].isnull().sum()
        if null_count > 0:
            print(f"Existing taxpayer name column '{taxpayer_col}' contains {null_count:,} null values - will supplement with registration data")
            perform_merge = True
        else:
            print(f"Existing taxpayer name column '{taxpayer_col}' is complete - no merge needed")
    else:
        print("No taxpayer name column found - will add from registration data")
        perform_merge = True
    ...
    if perform_merge:
        try:
            ...
            registration_data = pd.read_csv(
                registration_file,
                dtype_backend='pyarrow'
            )
            
            # Standardize column names in registration data
            registration_data.columns = registration_data.columns.str.lower().str.replace(' ', '_')
            
            # Check if required columns exist in registration data
            if 'tin' not in registration_data.columns:
                raise KeyError("TIN column not found in registration data")
            
            # Try to find taxpayer name column
            alt_names = ['taxpayer_name', 'taxpayername', 'name', 'tax_payer_name', 'taxpayer']
            taxpayer_name_col = None
            for name in alt_names:
                if name in registration_data.columns:
                    taxpayer_name_col = name
                    break
            
            if taxpayer_name_col is None:
                raise KeyError("Taxpayer name column not found in registration data")
            
            # Clean and prepare registration data - use vectorized operations
            print("Preparing registration data...")
            registration_data = registration_data[['tin', taxpayer_name_col]].copy()
            registration_data = registration_data.drop_duplicates()
            
            # Use vectorized string operations
            registration_data['tin'] = registration_data['tin'].astype(str).str.strip()
            
            # Prepare CIT data
            if 'tin' not in df.columns:
                print("Warning: TIN column not found in CIT data. Cannot merge with registration data.")
                return df
            
            print("Merging with CIT data...")
            # Use vectorized operations for TIN cleaning
            df['tin'] = df['tin'].astype(str).str.strip()
            
            # Optimize merge by creating dictionary mapping
            tin_to_name = dict(zip(registration_data['tin'], registration_data[taxpayer_name_col]))
            
            if existing_taxpayer_cols:
                # Fill nulls using map for better performance
                existing_col = existing_taxpayer_cols[0]
                mask = df[existing_col].isnull()
                df.loc[mask, existing_col] = df.loc[mask, 'tin'].map(tin_to_name)
                print(f"Null values in '{existing_col}' filled from registration data")
            else:
                # Add new column using map
                df['taxpayer_name'] = df['tin'].map(tin_to_name)
                print("Taxpayer name column added from registration data")
                
        except FileNotFoundError:
            print(f"Warning: Registration file '{registration_file}' not found. Skipping name enrichment.")
        except Exception as e:
            print(f"Warning: Could not process registration data: {str(e)}")
...
def reorganize_columns(df):
    ...
    possible_taxpayer_set = {'taxpayer', 'taxpayername', 'taxpayer_name', 'taxpayer', 'taxpayername'}
    possible_tin_set = {'tin', 'taxid', 'tax_id', 'taxidentificationnumber', 'taxpayerid'}
    
    existing_taxpayer_col = None
    existing_tin_col = None
    
    # Single pass through columns
    for col in tqdm(df.columns, desc="Reorganizing", unit="col"):
        col_normalized = col.lower().replace('_', '')
        if col_normalized in possible_taxpayer_set and existing_taxpayer_col is None:
            existing_taxpayer_col = col
        elif col_normalized in possible_tin_set and existing_tin_col is None:
            existing_tin_col = col
    
    # Default to 'tin' if not found
    if existing_tin_col is None and 'tin' in df.columns:
        existing_tin_col = 'tin'
    
    # Get all other columns (excluding taxpayer and TIN columns)
    columns_to_exclude = set()
    if existing_taxpayer_col:
        columns_to_exclude.add(existing_taxpayer_col)
    if existing_tin_col:
        columns_to_exclude.add(existing_tin_col)
    
    other_columns = [col for col in df.columns if col not in columns_to_exclude]
    
    # Reconstruct DataFrame with optimized column order
    column_order = []
    if existing_tin_col:
        column_order.append(existing_tin_col)
    if existing_taxpayer_col:
        column_order.append(existing_taxpayer_col)
